...
class DistributedModel:
    ...
    def set_tortuous_parameter(self, lambda_perp,
        ...
        # Attach tortuosity citation and constraint for the citation graph
        if not hasattr(self, '_tortuosity_citations'):
            self._tortuosity_citations = []
        self._tortuosity_citations.append(
            {'key': 'szafer1995', 'authors': 'Szafer A, Zhong J, Gore JC',
             'title': 'Theoretical model for water diffusion in tissues',
             'journal': 'Magnetic Resonance in Medicine',
             'year': 1995, 'doi': '10.1002/mrm.1910330708'})
        if not hasattr(self, '_tortuosity_constraints'):
            self._tortuosity_constraints = []
        self._tortuosity_constraints.append(
            {'id': 'tortuosity_assumption',
             'name': 'Tortuosity constraint',
             'condition_human': 'Extra-neurite perpendicular diffusivity linked '
                                'to parallel diffusivity via D_perp = D_par * '
                                '(1 - f_intra). Assumes well-mixed extra-axonal '
                                'space with tortuosity determined solely by '
                                'intra-neurite volume fraction.',
             'severity': 'warning',
             'source_key': 'szafer1995'})
    ...
    def sh_convolved_model(self, acquisition_scheme, **kwargs):
        ...
        For waveform-based acquisition schemes (those with a stored G(t) array,
        i.e. acquisition_scheme._G is not None), a direct SH inner product path
        is used: each model must implement signal_lm(scheme, **params) returning
        E_lm of shape (n_m, 45), and the dispersed signal is E_lm @ odf_lm.
        This path is exact for arbitrary rotating waveforms (STE / PTE and other
        tensor-valued encodings) where the fiber response is not axisymmetric.

        For PGSE schemes (acquisition_scheme._G is None), the classical
        rotational-harmonics (RH) convolution path is used: the model's
        rotational_harmonics_representation() is convolved with the ODF SH
        per shell.

        Parameters
        ----------
        acquisition_scheme : AcquisitionScheme or PGSEAcquisitionScheme,
            An acquisition scheme instantiated using dMipy.
        kwargs: keyword arguments to the model parameter values,
            Is internally given as **parameter_dictionary.
        """
        ...
        for parameter in self.distribution.parameter_ranges:
            ...
            distribution_parameters[parameter] = kwargs.get(parameter_name)
        ...
        # --- Waveform path: SH inner product E_lm @ odf_lm ---
        use_waveform_path = (
            hasattr(acquisition_scheme, '_G')
            and acquisition_scheme._G is not None
        )

        if use_waveform_path:
            E = np.zeros(acquisition_scheme.number_of_measurements)
            odf_lm = np.asarray(sh_distribution, dtype=np.float64)

            for model_name, model, partial_volume in zip(
                    self.model_names, self.models,
                    chain(partial_volumes, [None])
            ):
                if not hasattr(model, 'signal_lm'):
                    raise AttributeError(
                        "Model '{}' does not implement signal_lm() and cannot "
                        "be used with a waveform-based acquisition scheme. "
                        "signal_lm(scheme) is required for dispersed-signal "
                        "computation with rotating/tensor-valued waveforms. "
                        "Use a PGSE scheme or add signal_lm() to the "
                ...
                for parameter in model.parameter_ranges:
                    ...
                    parameters[parameter] = kwargs.get(parameter_name)
                ...
                E_lm = model.signal_lm(acquisition_scheme, **parameters)
                n = min(E_lm.shape[1], len(odf_lm))
                S = E_lm[:, :n] @ odf_lm[:n]   # (n_m,)

                T2 = parameters.get('T2')
                if (T2 is not None
                        and not np.isnan(np.asarray(T2, dtype=float).flat[0])
                        and acquisition_scheme.TE is not None):
                    S = S * np.exp(-acquisition_scheme.TE / T2)

                E += volume_fraction * S
            return E

        # --- PGSE path: classical rotational-harmonics convolution ---
        rh_models = 0.
        vf_t2_pairs = []  # (volume_fraction, T2) for compartments with T2
        for model_name, model, partial_volume in zip(
                self.model_names, self.models,
                chain(partial_volumes, [None])
        ):
            ...
            if (T2 is not None and
                    not np.isnan(np.asarray(T2, dtype=float).flat[0]) and
                    acquisition_scheme.TE is not None):
                dwi_TEs = acquisition_scheme.shell_TE[
                    acquisition_scheme.unique_dwi_indices]
                rh_model = rh_model * np.exp(-dwi_TEs / T2)[:, None]
                vf_t2_pairs.append((volume_fraction, T2))
            rh_models = rh_models + volume_fraction * rh_model

        E = np.ones(acquisition_scheme.number_of_measurements)
        # apply T2 weighting to b0 measurements (DWI shells handled via rh_model)
        if vf_t2_pairs and acquisition_scheme.TE is not None:
            b0_mask = acquisition_scheme.b0_mask
            b0_TEs = acquisition_scheme.TE[b0_mask]
            total_t2_vf_weighted = sum(
                vf * np.exp(-b0_TEs / t2) for vf, t2 in vf_t2_pairs)
            total_vf_with_t2 = sum(vf for vf, _ in vf_t2_pairs)
            E[b0_mask] = total_t2_vf_weighted + (1. - total_vf_with_t2)
    ...
    def integrated_model(self, acquisition_scheme, **kwargs):
        ...
        for model_name, model, partial_volume in zip(
            ...
            E = np.trapezoid(E, x=radii, axis=0)
...
class DD2PoissonDistributed(DistributedModel, AnisotropicSignalModelProperties):
    """
    The DistributedModel instantiation for a Poisson-parameterized diameter
    distribution for cylinder models. Uses :class:`DD2Poisson` (Gamma with
    fixed scale = 1 µm) as the diameter distribution, replacing the 'diameter'
    parameter of the wrapped model with a single free parameter 'mu' (mean
    diameter in meters).

    After instantiation the PoissonDistributed model can be treated exactly the
    same as a CompartmentModel as an input for a MultiCompartmentModel.

    Parameters
    ----------
    models : list
        List of models to be distributed (typically a single cylinder model).
    parameter_links : list, optional
        Deprecated parameter linking mechanism (testing use only).
    target_parameter : str
        Name of the model parameter to be replaced by the distribution.
    """
    _model_type = 'SpatialDistributedModel'
    ...
    def __init__(self, models, parameter_links=None,
        ...
        self.normalization = models[0].parameter_types[target_parameter]
        self.distribution = distributions.DD2Poisson(
            normalization=self.normalization)
