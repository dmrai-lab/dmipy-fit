...
import numpy as np
from dmipy_fit.signal_models import cylinder_models, plane_models
...
def test_RTAP_to_diameter_callaghan(samples=10000):
    ...
    rtap_callaghan = 2 * np.pi * np.trapezoid(
...
def test_RTPP_to_length_callaghan(samples=1000):
    ...
    rtpp_callaghan = 2 * np.trapezoid(E_callaghan, x=qvals_perp)
