...
import numpy as np
from dmipy_fit.signal_models import plane_models, cylinder_models, sphere_models
...
def test_RTAP_to_diameter_soderman(samples=1000):
    ...
    rtap_soderman = 2 * np.pi * np.trapezoid(
...
def test_RTOP_to_diameter_soderman(samples=1000):
    ...
    rtop_soderman = 4 * np.pi * np.trapezoid(
