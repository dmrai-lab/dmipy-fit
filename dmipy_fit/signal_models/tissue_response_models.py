import numpy as np
from dmipy_fit.utils.utils import cart2mu
...
def estimate_TR1_isotropic_tissue_response_model(acquisition_scheme, data):
    ...
    kernel of e.g. CSF or grey matter from array of candidate voxels.
...
def estimate_TR2_anisotropic_tissue_response_model(
        acquisition_scheme, data):
    """
    Estimates TR2 anistropic TissueResponseModel describing the convolution
    kernel of e.g. anistropic white matter from array of candidate voxels
...
class TR1IsotropicTissueResponseModel(
    ...
    representation of some signal attenuation.
    """
    _citations = {
        'definition': [
            {'key': 'tournier2013', 'authors': 'Tournier J-D, Calamante F, Connelly A',
             ...
             'year': 2013, 'doi': '10.1002/nbm.2903'}
        ],
        'default_parameters': {},
    }
    _validity_constraints = [
        {'id': 'voxel_selection_quality',
         'name': 'Input voxel selection quality',
         'condition_human': 'The tissue response is derived by averaging signals from voxels segmented as the target tissue type (WM/GM/CSF). No outlier rejection is applied. The response validity depends on the input voxels being representative of a single tissue population -- mixing single-bundle and crossing-fiber voxels in the WM response will bias it.',
         'severity': 'warning'},
        {'id': 'macroscopic_signal_average',
         'name': 'Macro-signal averaging',
         'condition_human': 'The response represents the average MACRO-SIGNAL of the selected voxels, which includes effects of sub-voxel orientation dispersion, partial volume, and microstructure heterogeneity. It is NOT a single-fiber response kernel -- it is the empirical average signal of voxels classified as that tissue type.',
         'severity': 'info'},
    ]
    r"""

    NOTE: T2 weighting is implicit in the empirical RH coefficients; adding an
    explicit T2 parameter would double-count the relaxation effect. For this
    reason, TR models intentionally do NOT have a T2 parameter. It can be instantiated with any
    rotational harmonics and accompanying PGSEAcquisitionScheme, but is
    ...
    def rotational_harmonics_representation(
        ...
        stored_fps = self.acquisition_scheme.shell_fingerprints
        stored_dwi_fps = [stored_fps[idx]
                          for idx in self.acquisition_scheme.unique_dwi_indices]
        for i, dwi_shell_index in enumerate(rh_scheme.unique_dwi_indices):
            query_fp = acquisition_scheme.shell_fingerprints[dwi_shell_index]
            matches = [j for j, fp in enumerate(stored_dwi_fps)
                       if (np.isclose(fp[0], query_fp[0], atol=1e6) and
                           np.isclose(fp[1], query_fp[1], atol=1.0))]
            if not matches:
                raise ValueError(
                    'Shell (b={:.0f}s/mm², f={:.0f}Hz) not in tissue response '
                    'shells. Available: {}'.format(
                        query_fp[0] / 1e6, query_fp[1],
                        [('{:.0f}s/mm²'.format(fp[0] / 1e6),
                          '{:.0f}Hz'.format(fp[1]))
                         for fp in stored_dwi_fps]))
            native_dwi_shell_index = matches[0]
