...
class FittedMultiCompartmentModel:
    ...
    def __init__(self, model, S0, mask, fitted_parameters_vector,
        ...
        self.fitted_sigma = fitted_sigma
...
class FittedMultiCompartmentSphericalHarmonicsModel:
    ...
    def fod(self, vertices):
        ...
        sh_matrix = real_sym_sh_mrtrix(self.model.sh_order, theta, phi, legacy=False)[0]
        dataset_shape = self.fitted_parameters_vector.shape[:-1]
        N_samples = len(vertices)
        fods = np.zeros(np.r_[dataset_shape, N_samples])
        mask_pos = np.where(self.mask)
    ...
    def _anisotropic_model_index(self):
        "Index of the single oriented (white-matter) tissue response."
        for i, m in enumerate(self.model.models):
            if 'orientation' in m.parameter_types.values():
                return i
        raise ValueError(
            "isolate_wm_signal requires one oriented (white-matter) tissue "
            "response in the multi-tissue model; none was found.")

    def _isolation_S0(self, S0):
        """S0 scaling for reconstructing the isotropic signal (matches predict)."""
        dataset_shape = self.fitted_parameters_vector.shape[:-1]
        if S0 is None:
            if self.fit_S0_response:
                return np.ones(dataset_shape) * self.max_S0_response
            return self.S0
        if isinstance(S0, float):
            if self.fit_S0_response:
                return S0 * self.max_S0_response / self.S0
            return np.ones(dataset_shape) * S0
        return S0

    def _isotropic_signal_voxel(self, pos, acquisition_scheme, S0):
        """Per-voxel isotropic (GM/CSF) signal: the multi-tissue prediction with
        the white-matter compartment (its FOD and volume fraction) zeroed, so
        only the data-derived isotropic responses x their fractions remain."""
        parameters = self.model.parameter_vector_to_parameters(
            self.fitted_parameters_vector[pos])
        parameters['sh_coeff'] = np.zeros_like(parameters['sh_coeff'])
        parameters[self.model.partial_volume_names[
            self._anisotropic_model_index()]] = 0.
        return self.model(acquisition_scheme,
                          **dict(parameters, **self._fit_parameters)) * S0[pos]

    def isolate_wm_signal(self, data, acquisition_scheme=None, S0=None,
                          mask=None):
        """
        Removes the isotropic (e.g. grey-matter and CSF) tissue contributions
        estimated by multi-tissue CSD from the data, returning the full
        per-measurement white-matter (anisotropic) signal alone.

        The grey-matter and CSF contributions are reconstructed from the
        multi-tissue fit's own data-derived (non-parametric) isotropic response
        functions, scaled by the fitted multi-tissue volume fractions and S0 -
        no Gaussian compartment is assumed for them. Subtracting them leaves
        the white-matter signal (plus residual).

        For the spherical-mean white-matter kernel fit, prefer
        :meth:`isolate_wm_spherical_mean`, which returns the (tiny) per-shell
        spherical mean directly and never materialises a full per-measurement
        white-matter volume.

        Parameters
        ----------
        data : array of size (..., N_DWIs),
            The (non-normalised) measured signal that was fitted.
        acquisition_scheme : AcquisitionScheme instance or None,
            Scheme to reconstruct the isotropic signal on. Defaults to the
            fitting scheme.
        S0 : None or float,
            Signal intensity without diffusion sensitisation. If None, uses the
            estimated S0 from the fit, matching ``predict``.
        mask : array or None,
            Voxels to operate on. Defaults to the fitting mask.

        Returns
        -------
        wm_signal : array of size (..., N_DWIs),
            data with the isotropic tissue contributions removed.
        """
        if acquisition_scheme is None:
            acquisition_scheme = self.model.scheme
        self.model._check_model_params_with_acquisition_params(
            acquisition_scheme)
        S0 = self._isolation_S0(S0)
        if mask is None:
            mask = self.mask

        # subtract the isotropic prediction in place, per voxel, into a single
        # output (peak memory ~ one brain, not two). broadcast_to + copy gives
        # the (dataset_shape, N_meas) destination for any input layout.
        N_samples = len(acquisition_scheme.bvalues)
        target = tuple(self.fitted_parameters_vector.shape[:-1]) + (N_samples,)
        wm_signal = np.broadcast_to(
            np.asarray(data, dtype=float), target).copy()
        for pos in zip(*np.where(mask)):
            wm_signal[pos] -= self._isotropic_signal_voxel(
                pos, acquisition_scheme, S0)
        return wm_signal

    def isolate_wm_spherical_mean(self, data, acquisition_scheme=None, S0=None,
                                  mask=None):
        """
        White-matter-only signal in spherical-mean (per-shell) space, with the
        isotropic GM/CSF contributions removed -- the input for the
        spherical-mean white-matter kernel fit.

        Because grey matter and CSF are isotropic, removing them in
        spherical-mean space is exact, and the result is only
        ``(..., N_shells)`` -- kilobytes per voxel -- so no full per-measurement
        white-matter volume is ever held and ``data`` is left untouched (fit
        quality / error metrics on the original signal still work). Feed the
        result straight to ``MultiCompartmentSphericalMeanModel.fit``, which
        accepts spherical-mean data directly.

        Parameters mirror :meth:`isolate_wm_signal`.

        Returns
        -------
        wm_spherical_mean : array of size (..., N_shells),
            per-shell spherical mean of the white-matter signal (b0 shell plus
            one value per diffusion-weighted shell).
        """
        ...
        dataset_shape = self.fitted_parameters_vector.shape[:-1]
        N_meas = len(acquisition_scheme.bvalues)
        data = np.broadcast_to(
            np.asarray(data, dtype=float),
            tuple(dataset_shape) + (N_meas,))
        wm_sm = np.zeros(np.r_[dataset_shape, acquisition_scheme.N_shells])
        for pos in zip(*np.where(mask)):
            iso_voxel = self._isotropic_signal_voxel(
                pos, acquisition_scheme, S0)
            wm_sm[pos] = (
                estimate_spherical_mean_multi_shell(data[pos], acquisition_scheme)
                - estimate_spherical_mean_multi_shell(
                    iso_voxel, acquisition_scheme))
        return wm_sm
