...
SPHERES_PATH = _os.path.join(_os.path.dirname(__file__), '..', 'data', 'spheres')
...
class GlobalBruteOptimizer:
    ...
    _citations = {
        'definition': [
            {'key': 'caruyer2013', 'authors': 'Caruyer E, Lenglet C, Sapiro G, Deriche R',
             'title': 'Design of multishell sampling schemes with uniform coverage in diffusion MRI',
             'journal': 'Magnetic Resonance in Medicine',
             'year': 2013, 'doi': '10.1002/mrm.24736'},
        ],
        'default_parameters': {},
    }
    ...
    def precompute_signal_grid(self, model, x0_vector, Ns, N_sphere_samples):
        ...
        for card_counter in range(max_cardinality):
            ...
            for name, card in parameter_cardinality_items:
                ...
                if card == 1:
                    if np.isnan(x0_vector[counter]) and not opt_flag:
                        # passive parameter: use midpoint, don't grid-search
                        midpoint = ((par_range[0] + par_range[1]) / 2. *
                                    model.parameter_scales[name])
                        per_parameter_vectors.append([midpoint])
                    elif np.isnan(x0_vector[counter]):
...
class Brute2FineOptimizer:
    ...
    def __call__(self, data, x0_vector):
        ...
        opt_flags = self.model.opt_params_for_optimization
        for i, x0_ in enumerate(x0_vector):
            if np.isnan(x0_) and opt_flags[i] is False:
                # passive parameter: use midpoint, fix fine bounds too
                midpoint = (bounds[i][0] + bounds[i][1]) / 2.
                bounds_brute.append(slice(midpoint, midpoint + 1e-2, None))
                bounds_fine[i] = np.r_[midpoint, midpoint]
            elif np.isnan(x0_):
                bounds_brute.append(
