...
def test_tissue_response_model_multi_compartment_models():
    ...
    # Fit should raise ValueError when given an incompatible scheme
    # (different shells than the tissue response was estimated on).
    bvals_incompatible = np.array([0., 3000e6, 5000e6, 10000e6])
    dirs_incompatible = np.tile([0., 0., 1.], (4, 1))
    scheme_incompatible = acquisition_scheme_from_bvalues(
        bvals_incompatible, dirs_incompatible)
    assert_raises(ValueError,
                  mc_csd.fit,
                  acquisition_scheme=scheme_incompatible,
                  data=data_watson)
